<?php
if ( !function_exists ( 'ninja_forms_post_meta_value_box' ) ) {
	function ninja_forms_post_meta_value_box( $field_id ){
		...
		if ( isset ( $form_row['data']['create_post'] ) AND $form_row['data']['create_post'] == 1 ) {
			...
			if( $field_process ){
				...
				?>
				<input type="text" name="ninja_forms_field_<?php echo $field_id;?>[post_meta_value]" id="ninja_forms_field_<?php echo $field_id;?>_post_custom_meta_value" value="<?php echo $post_meta_value;?>" style="<?php echo $display_input;?>">
				</div>
				<?php
			}	
		}
